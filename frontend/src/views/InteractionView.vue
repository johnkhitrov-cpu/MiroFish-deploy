<template>
  <div class="main-view">
    ...
    <header class="app-header">
      ...
      <div class="header-right">
        <div class="workflow-step">
          <span class="step-num">Шаг 5/5</span>
          <span class="step-name">Глубокое взаимодействие</span>
        </div>
    ...
    <main class="content-area">
      ...
      <!-- Right Panel: Step5 Глубокое взаимодействие -->
...
// Layout State - По умолчанию переключаемся на вид рабочего места
...
const statusText = computed(() => {
  ...
  if (currentStatus.value === 'processing') return 'Обработка'
  return 'Готово'
})
...
const loadReportData = async () => {
  try {
    addLog(`Загрузка данных отчёта: ${currentReportId.value}`)
    
    // Получение информации об отчёте для получения simulation_id
    ...
    if (reportRes.success && reportRes.data) {
      ...
      addLog(`Ошибка получения информации об отчёте: ${reportRes.error || 'неизвестная ошибка'}`)
    }
  } catch (err) {
...
onMounted(() => {
  addLog('Инициализация InteractionView')
  loadReportData()
})
</script>
