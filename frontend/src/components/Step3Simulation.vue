  ...
  <div class="simulation-panel">
    ...
    <div class="control-bar">
      <div class="status-group">
        ...
        <!-- Прогресс платформы Reddit -->
        <div class="platform-status reddit" :class="{ active: runStatus.reddit_running, completed: runStatus.reddit_completed }">
          <div class="platform-header">
            <svg class="platform-icon" viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2">
              <path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"></path>
            </svg>
            <span class="platform-name">Тематическое сообщество</span>
    ...
    <div class="main-content-area" ref="scrollContainer">
      ...
      <div class="timeline-header" v-if="allActions.length > 0">
        <div class="timeline-stats">
          <span class="total-count">ВСЕГО СОБЫТИЙ: <span class="mono">{{ allActions.length }}</span></span>
              ...
              <div class="card-body">
                ...
                <template v-if="action.action_type === 'QUOTE_POST'">
                  ...
                  <div v-if="action.action_args?.original_content" class="quoted-block">
                    <div class="quote-header">
                      ...
                      <span class="quote-label">@{{ action.action_args.original_author_name || 'Пользователь' }}</span>
                    </div>
                ...
                <template v-if="action.action_type === 'REPOST'">
                  <div class="repost-info">
                    ...
                    <span class="repost-label">Репост от @{{ action.action_args?.original_author_name || 'Пользователь' }}</span>
                  </div>
                ...
                <template v-if="action.action_type === 'LIKE_POST'">
                  <div class="like-info">
                    ...
                    <span class="like-label">Лайк @{{ action.action_args?.post_author_name || 'Пользователь' }}</span>
                  </div>
                ...
                <template v-if="action.action_type === 'CREATE_COMMENT'">
                  ...
                  <div v-if="action.action_args?.post_id" class="comment-context">
                    ...
                    <span>Ответ на пост #{{ action.action_args.post_id }}</span>
                  </div>
                </template>
                ...
                <template v-if="action.action_type === 'SEARCH_POSTS'">
                  <div class="search-info">
                    ...
                    <span class="search-label">Поисковый запрос:</span>
                ...
                <template v-if="action.action_type === 'FOLLOW'">
                  <div class="follow-info">
                    ...
                    <span class="follow-label">Подписка на @{{ action.action_args?.target_user || action.action_args?.user_id || 'Пользователь' }}</span>
                  </div>
                </template>
                ...
                <template v-if="action.action_type === 'UPVOTE_POST' || action.action_type === 'DOWNVOTE_POST'">
                  <div class="vote-info">
                    ...
                    <span class="vote-label">{{ action.action_type === 'UPVOTE_POST' ? 'Голос за' : 'Голос против' }}</span>
                  </div>
                ...
                <!-- DO_NOTHING: бездействие -->
                <template v-if="action.action_type === 'DO_NOTHING'">
                  <div class="idle-info">
                    <svg class="icon-small" viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="12"></line><line x1="12" y1="16" x2="12.01" y2="16"></line></svg>
                    <span class="idle-label">Действие пропущено</span>
                  </div>
                </template>

                <!-- Общий fallback: неизвестный тип или content, не обработанный выше -->
    ...
    <div class="system-logs">
      <div class="log-header">
        <span class="log-title">МОНИТОР СИМУЛЯЦИИ</span>
...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // макс. кол-во раундов из Step2
  minutesPerRound: {
    type: Number,
    default: 30 // по умолчанию 30 минут на раунд
  },
  projectData: Object,
  graphData: Object,
  systemLogs: Array
...
const phase = ref(0) // 0: не начато, 1: выполняется, 2: завершено
const isStarting = ref(false)
const isStopping = ref(false)
const startError = ref(null)
const runStatus = ref({})
const allActions = ref([]) // все действия (инкрементальное накопление)
const actionIds = ref(new Set()) // набор ID действий для дедупликации
const scrollContainer = ref(null)

// Computed
// Отображение действий в хронологическом порядке (новейшие внизу)
const chronologicalActions = computed(() => {
  return allActions.value
})

// Счётчики действий по платформам
...
// Форматирование прошедшего времени симуляции (по раундам и минутам на раунд)
...
// Прошедшее время симуляции платформы Twitter
const twitterElapsedTime = computed(() => {
  return formatElapsedTime(runStatus.value.twitter_current_round || 0)
})

// Прошедшее время симуляции платформы Reddit
...
// Сброс всех состояний (для перезапуска симуляции)
const resetAllState = () => {
  phase.value = 0
  runStatus.value = {}
  allActions.value = []
  ...
  stopPolling()  // остановка возможного предыдущего опроса
}
...
const doStartSimulation = async () => {
  ...
  // Сброс всех состояний, чтобы не было влияния предыдущей симуляции
  resetAllState()
  
  isStarting.value = true
  startError.value = null
  addLog('Запуск параллельной двухплатформенной симуляции...')
  ...
  try {
    const params = {
      ...
      enable_graph_memory_update: true  // включить динамическое обновление графа
    }
    
    if (props.maxRounds) {
      params.max_rounds = props.maxRounds
      addLog(`Установлено макс. кол-во раундов: ${props.maxRounds}`)
    }
    
    addLog('Режим динамического обновления графа включён')
    
    const res = await startSimulation(params)
    
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ Старые логи симуляции очищены, перезапуск')
      }
      addLog('✓ Движок симуляции запущен успешно')
    ...
    addLog(`✗ Исключение при запуске: ${err.message}`)
    emit('update-status', 'error')
  } finally {
    isStarting.value = false
  }
...
const handleStopSimulation = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      addLog(`Ошибка остановки: ${res.error || 'Неизвестная ошибка'}`)
    }
  } catch (err) {
    addLog(`Исключение при остановке: ${err.message}`)
  } finally {
    isStopping.value = false
  }
}
...
// Отслеживание предыдущих раундов платформ для обнаружения изменений
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Проверка изменений раундов по платформам и вывод логов
      ...
      // Проверка завершения симуляции (по runner_status или статусу платформ)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Доп. проверка: бэкенд может не обновить runner_status, а платформы уже завершены
      // Определяем по twitter_completed и reddit_completed
      const platformsCompleted = checkPlatformsCompleted(data)
      
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ Обнаружено завершение симуляции всех платформ')
        }
    ...
    console.warn('Ошибка получения статуса выполнения:', err)
  }
}

// Проверка завершения всех включённых платформ
const checkPlatformsCompleted = (data) => {
  // Нет данных платформ — false
  if (!data) return false
  
  // Проверка статуса завершения платформ
  const twitterCompleted = data.twitter_completed === true
  const redditCompleted = data.reddit_completed === true
  
  // Если хотя бы одна платформа завершена — проверяем, все ли завершены
  // Определяем включённость платформы по actions_count (count > 0 или running было true)
  const twitterEnabled = (data.twitter_actions_count > 0) || data.twitter_running || twitterCompleted
  const redditEnabled = (data.reddit_actions_count > 0) || data.reddit_running || redditCompleted
  
  // Нет включённых платформ — false
  if (!twitterEnabled && !redditEnabled) return false
  
  // Проверка, все ли включённые платформы завершены
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // Получение полного списка действий через all_actions
      const serverActions = res.data.all_actions || []
      
      // Инкрементальное добавление новых действий (дедупликация)
      ...
      // Без автопрокрутки, пользователь свободно просматривает таймлайн
      // Новые действия добавляются внизу
    }
  } catch (err) {
    console.warn('Ошибка получения детального статуса:', err)
  }
}
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('Запрос на генерацию отчёта отправлен, подождите...')
    return
  }
  
  isGeneratingReport.value = true
  addLog('Запуск генерации отчёта...')
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✓ Задача генерации отчёта запущена: ${reportId}`)
      
      // Переход на страницу отчёта
      router.push({ name: 'Report', params: { reportId } })
    } else {
      addLog(`✗ Ошибка запуска генерации отчёта: ${res.error || 'Неизвестная ошибка'}`)
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(`✗ Исключение при запуске генерации отчёта: ${err.message}`)
    isGeneratingReport.value = false
  }
}
...
onMounted(() => {
  addLog('Step3 инициализация запуска симуляции')
  if (props.simulationId) {
    doStartSimulation()
  }
})
