            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (контент сгенерирован, но раздел может быть ещё не завершён) -->
                  ...
                  <!-- Section Complete (генерация раздела завершена) -->
...
const toggleRawResult = (timestamp, event) => {
  // Сохранение позиции кнопки относительно viewport
  ...
  // После обновления DOM корректировка прокрутки для сохранения позиции кнопки
...
const toggleSectionCollapse = (idx) => {
  // Сворачивать можно только завершённые разделы
  if (!generatedSections.value[idx + 1]) return
...
const parseInsightForge = (text) => {
  ...
  try {
    // Извлечение вопроса анализа
    const queryMatch = text.match(/分析问题:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // Извлечение сценария прогноза
    const reqMatch = text.match(/预测场景:\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()
    
    // Извлечение статистики — формат "相关预测事实: X条"
    ...
    // Извлечение подвопросов — полное извлечение без ограничения количества
    ...
    // Извлечение ключевых фактов — полное извлечение без ограничения количества
    ...
    // Извлечение ключевых сущностей — полное извлечение с аннотациями и количеством связанных фактов
    const entitySection = text.match(/### 【核心实体】\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
      const entityText = entitySection[1]
      // Разделение блоков сущностей по "- **"
    ...
    // Извлечение цепочек связей — полное извлечение без ограничения количества
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Извлечение текущих активных фактов — полное извлечение без ограничения количества
    const activeSection = text.match(/### 【当前有效事实】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
      const lines = activeSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.activeFacts = lines.map(l => {
        // Удаление нумерации и кавычек
        const factText = l.replace(/^\d+\.\s*/, '').replace(/^"|"$/g, '').trim()
        return factText
      }).filter(Boolean)
    }
    
    // Извлечение исторических/устаревших фактов — полное извлечение без ограничения количества
    ...
    // Извлечение связанных сущностей — полное извлечение без ограничения количества
...
const parseInterview = (text) => {
  ...
  try {
    // Извлечение темы интервью
    const topicMatch = text.match(/\*\*采访主题:\*\*\s*(.+?)(?:\n|$)/)
    if (topicMatch) result.topic = topicMatch[1].trim()
    
    // Извлечение количества интервьюируемых (напр. "5 / 9 симулированных Agent")
    ...
    // Извлечение обоснования выбора интервьюируемых
    const reasonMatch = text.match(/### 采访对象选择理由\n([\s\S]*?)(?=\n---\n|\n### 采访实录)/)
    if (reasonMatch) {
      result.selectionReason = reasonMatch[1].trim()
    }
    
    // Разбор обоснования выбора каждого респондента
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Формат 1: число. **имя（index=X）**: причина
        // Пример: 1. **校友_345（index=1）**: как выпускник...
        ...
        // Формат 2: - Выбор имени（index X）: причина
        // Пример: - Выбор家长_601（index 0）: как представитель родителей...
        if (!headerMatch) {
        ...
        // Формат 3: - **имя（index X）**: причина
        // Пример: - **家长_601（index 0）**: как представитель родителей...
        if (!headerMatch) {
        ...
        if (name) {
          // Сохранение обоснования предыдущего респондента
          if (currentName && currentReason.length > 0) {
            reasons[currentName] = currentReason.join(' ').trim()
          }
          // Начало нового респондента
          currentName = name
          currentReason = reasonStart ? [reasonStart.trim()] : []
        } else if (currentName && line.trim() && !line.match(/^未选|^综上|^最终选择/)) {
          // Продолжение обоснования (исключая итоговые абзацы)
          currentReason.push(line.trim())
        }
      }
      
      // Сохранение обоснования последнего респондента
      if (currentName && currentReason.length > 0) {
        reasons[currentName] = currentReason.join(' ').trim()
      }
    ...
    // Извлечение каждой записи интервью
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (qMatch) {
        ...
        // Разделение вопросов по числовым номерам
        const questions = qText.split(/\n\d+\.\s+/).filter(q => q.trim())
        if (questions.length > 0) {
          // Если перед первым вопросом стоит "1.", требуется особая обработка
      ...
      // Извлечение ответов — разделение по Twitter и Reddit
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*关键引言|$)/)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()
        
        // Разделение ответов Twitter и Reddit
        ...
        // Логика отката платформы (совместимость со старым форматом: одна платформа)
        if (!twitterMatch && redditMatch) {
          // Только ответ Reddit — копирование как отображение по умолчанию (если не заглушка)
          ...
          // Нет разделения по платформам (устаревший формат) — весь текст как ответ
          interview.twitterAnswer = answerText
        }
      }
      
      // Извлечение ключевых цитат (поддержка различных форматов кавычек)
      const quotesMatch = block.match(/\*\*关键引言:\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
        const quotesText = quotesMatch[1]
        // Приоритетное совпадение формата > "text"
        let quoteMatches = quotesText.match(/> "([^"]+)"/g)
        // Откат: совпадение > "text" или > 201Ctext201D (китайские кавычки)
        if (!quoteMatches) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // Извлечение связанных фактов — полное извлечение без ограничения количества
    ...
    // Попытка извлечения информации о рёбрах (если есть)
    ...
    // Попытка извлечения информации об узлах (если есть)
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-header' }, [
        ...
        props.result.simulationRequirement && h('div', { class: 'header-scenario' }, [
          h('span', { class: 'scenario-label' }, 'Сценарий прогноза: '),
      ...
      h('div', { class: 'insight-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Текущая ключевая память (${props.result.facts.length})`)
        ]),
        h('button', {
          class: ['insight-tab', { active: activeTab.value === 'entities' }],
          onClick: () => { activeTab.value = 'entities' }
        }, [
          h('span', { class: 'tab-label' }, `Ключевые сущности (${props.result.entities.length})`)
        ]),
        h('button', {
          class: ['insight-tab', { active: activeTab.value === 'relations' }],
          onClick: () => { activeTab.value = 'relations' }
        }, [
          h('span', { class: 'tab-label' }, `Цепочки связей (${props.result.relations.length})`)
        ]),
        props.result.subQueries.length > 0 && h('button', {
          ...
          h('span', { class: 'tab-label' }, `Подвопросы (${props.result.subQueries.length})`)
        ])
      ]),
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Последние ключевые факты из темпоральной памяти'),
            h('span', { class: 'panel-count' }, `всего ${props.result.facts.length} шт.`)
          ]),
          ...
          }, expandedFacts.value ? `Свернуть ▲` : `Развернуть все ${props.result.facts.length} шт. ▼`)
        ]),
        ...
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          ...
          h('div', { class: 'entities-grid' },
            (expandedEntities.value ? props.result.entities : props.result.entities.slice(0, 12)).map((entity, i) => 
              h('div', { class: 'entity-tag', key: i, title: entity.summary || '' }, [
                ...
                entity.relatedFactsCount > 0 && h('span', { class: 'entity-fact-count' }, `${entity.relatedFactsCount} шт.`)
              ])
            )
          ),
          ...
          }, expandedEntities.value ? `Свернуть ▲` : `Развернуть все ${props.result.entities.length} шт. ▼`)
        ]),
        
        // Relations Tab
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Цепочки связей'),
            h('span', { class: 'panel-count' }, `всего ${props.result.relations.length} шт.`)
          ]),
          ...
          }, expandedRelations.value ? `Свернуть ▲` : `Развернуть все ${props.result.relations.length} шт. ▼`)
        ]),
        
        // Sub-queries Tab
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Генерация аналитических подвопросов дрейфовым запросом'),
        ...
        // Empty state
        activeTab.value === 'facts' && props.result.facts.length === 0 && h('div', { class: 'empty-state' }, 'Нет текущей ключевой памяти'),
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, 'Нет ключевых сущностей'),
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, 'Нет цепочек связей')
      ])
    ])
  }
}
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Текущая активная память (${props.result.activeFacts.length})`)
        ]),
        h('button', {
          class: ['panorama-tab', { active: activeTab.value === 'historical' }],
          onClick: () => { activeTab.value = 'historical' }
        }, [
          h('span', { class: 'tab-label' }, `Историческая память (${props.result.historicalFacts.length})`)
        ]),
        h('button', {
          class: ['panorama-tab', { active: activeTab.value === 'entities' }],
          onClick: () => { activeTab.value = 'entities' }
        }, [
          h('span', { class: 'tab-label' }, `Связанные сущности (${props.result.entities.length})`)
        ])
      ]),
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Текущая активная память'),
            h('span', { class: 'panel-count' }, `всего ${props.result.activeFacts.length} шт.`)
          ]),
          ...
          ) : h('div', { class: 'empty-state' }, 'Нет текущей активной памяти'),
          props.result.activeFacts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedActive.value = !expandedActive.value }
          }, expandedActive.value ? `Свернуть ▲` : `Развернуть все ${props.result.activeFacts.length} шт. ▼`)
        ]),
        
        // Historical Facts Tab
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Историческая память'),
            h('span', { class: 'panel-count' }, `всего ${props.result.historicalFacts.length} шт.`)
          ]),
            ...
            (expandedHistorical.value ? props.result.historicalFacts : props.result.historicalFacts.slice(0, INITIAL_SHOW_COUNT)).map((fact, i) => 
              h('div', { class: 'fact-item historical', key: i }, [
                ...
                h('div', { class: 'fact-content' }, [
                  // Попытка извлечения временной информации [time - time]
                  (() => {
          ...
          ) : h('div', { class: 'empty-state' }, 'Нет исторической памяти'),
          props.result.historicalFacts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedHistorical.value = !expandedHistorical.value }
          }, expandedHistorical.value ? `Свернуть ▲` : `Развернуть все ${props.result.historicalFacts.length} шт. ▼`)
        ]),
        ...
        activeTab.value === 'entities' && h('div', { class: 'entities-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'Нет связанных сущностей'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Поддержка независимого состояния выбора платформы для каждой пары вопрос-ответ
    const platformTabs = reactive({}) // { 'agentIdx-qIdx': 'twitter' | 'reddit' }
    
    // Получение текущего выбора платформы для вопроса
    const getPlatformTab = (agentIdx, qIdx) => {
      const key = `${agentIdx}-${qIdx}`
      return platformTabs[key] || 'twitter'
    }
    
    // Установка выбора платформы для вопроса
    ...
    // Проверка, является ли текст заглушкой платформы
    ...
    // Попытка разделения ответа по номерам вопросов
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Поддержка двух форматов нумерации:
      // 1. "问题X：" или "问题X:" (китайский формат, новый формат бэкенда)
      // 2. "1. " или "\n1. " (число+точка, совместимость со старым форматом)
      let matches = []
      let match

      // Приоритетная попытка формата "问题X："
      const cnPattern = /(?:^|[\r\n]+)问题(\d+)[：:]\s*/g
      ...
      // Если не найдено, откат к формату "число."
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      ...
      // Если номера не найдены или найден только один, вернуть целиком
      if (matches.length <= 1) {
    ...
    // Получение ответа, соответствующего вопросу
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Разделение успешно и индекс валиден
      if (answers.length > 1 && qIdx < answers.length) {
        return answers[qIdx] || ''
      }

      // Ошибка разделения: первый вопрос получает полный ответ, остальные — пусто
      return qIdx === 0 ? answer : ''
    }
    
    // Проверка наличия ответов с двух платформ для вопроса (с фильтрацией заглушек)
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Обе платформы имеют реальные ответы (не заглушки) и контент различается
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Переключатель платформ (показывается только при наличии реальных ответов с двух платформ)
                  ...
                  // Expand/Collapse Button (не отображается для заглушек)
    ...
    return () => h('div', { class: 'quick-search-display' }, [
      ...
      h('div', { class: 'quicksearch-header' }, [
        ...
        props.result.query && h('div', { class: 'header-query' }, [
          h('span', { class: 'query-label' }, 'Поиск: '),
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        ((!showTabs.value) || activeTab.value === 'facts') && h('div', { class: 'facts-panel' }, [
          !showTabs.value && h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Результаты поиска'),
            h('span', { class: 'panel-count' }, `всего ${props.result.facts.length} шт.`)
          ]),
          ...
          ) : h('div', { class: 'empty-state' }, 'Результаты не найдены'),
          props.result.facts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedFacts.value = !expandedFacts.value }
          }, expandedFacts.value ? `Свернуть ▲` : `Развернуть все ${props.result.facts.length} шт. ▼`)
        ...
        activeTab.value === 'edges' && hasEdges.value && h('div', { class: 'edges-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Связанные отношения'),
            h('span', { class: 'panel-count' }, `всего ${props.result.edges.length} шт.`)
          ]),
        ...
        activeTab.value === 'nodes' && hasNodes.value && h('div', { class: 'nodes-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Связанные узлы'),
...
// Текущий активный шаг (для отображения вверху)
const activeStep = computed(() => {
  const steps = workflowSteps.value
  // Поиск текущего активного шага
  const active = steps.find(s => s.status === 'active')
  if (active) return active
  
  // Если нет активного, вернуть последний завершённый шаг
...
const renderMarkdown = (content) => {
  ...
  // Удаление заголовка второго уровня (## xxx), т.к. заголовок раздела уже отображается снаружи
  ...
  // Обработка списков — поддержка вложенных списков
  ...
  // Обёртка маркированного списка
  html = html.replace(/(<li class="md-li"[^>]*>.*?<\/li>\s*)+/g, '<ul class="md-ul">$&</ul>')
  // Обёртка нумерованного списка
  html = html.replace(/(<li class="md-oli"[^>]*>.*?<\/li>\s*)+/g, '<ol class="md-ol">$&</ol>')

  // Очистка пробелов между элементами списка
  html = html.replace(/<\/li>\s+<li/g, '</li><li')
  // Очистка пробелов после открывающего тега списка
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // Очистка пробелов перед закрывающим тегом списка
  ...
  // Обработка переносов строк — пустая строка = разделитель абзацев, одинарный перенос = <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Очистка тегов <br> до и после блочных элементов
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // Очистка <p><br> перед блочными элементами (лишние пустые строки)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Очистка последовательных тегов <br>
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Очистка <br> перед открывающим тегом абзаца после блочного элемента
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Исправление нумерации прерывистых упорядоченных списков: сохранение последовательности при разделении абзацами
...
const getLogLevelClass = (log) => {
  ...
  if (log.includes('WARNING') || log.includes('предупреждение')) return 'warning'
  // INFO использует цвет по умолчанию, не помечается как success
  return ''
}
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          // section_complete — генерация раздела завершена
          if (log.action === 'section_complete') {
            if (log.details?.content) {
              generatedSections.value[log.section_index] = log.details.content
              // Автоматическое разворачивание только что сгенерированного раздела
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // Гарантия сброса состояния загрузки
            emit('update-status', 'completed')
            stopPolling()
            // Логика прокрутки обрабатывается в nextTick после завершения цикла
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // Если задача завершена — прокрутка вверх; иначе — вниз к последнему логу
            if (isComplete.value) {
...
// Извлечение финального ответа — извлечение содержимого раздела из ответа LLM
const extractFinalContent = (response) => {
  if (!response) return null
  
  // Попытка извлечения содержимого тега <final_answer>
  ...
  // Попытка найти содержимое после Final Answer: (поддержка нескольких форматов)
  // Формат 1: Final Answer:\n\nсодержимое
  // Формат 2: Final Answer: содержимое
  const finalAnswerMatch = response.match(/Final\s*Answer:\s*\n*([\s\S]*)$/i)
  if (finalAnswerMatch) {
    return finalAnswerMatch[1].trim()
  }
  
  // Попытка найти содержимое после 最终答案:
  const chineseFinalMatch = response.match(/最终答案[:：]\s*\n*([\s\S]*)$/i)
  if (chineseFinalMatch) {
    return chineseFinalMatch[1].trim()
  }
  
  // Если начинается с ## или # или > — возможно, прямой markdown
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // Если содержимое длинное с markdown — попытка удалить процесс размышления
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Удаление процесса размышления, начинающегося с Thought:
...
.section-number {
  ...
  font-size: 16px;
  color: #9CA3AF; /* Тёмно-серый, не меняется в зависимости от состояния */
  font-weight: 500;
}
...
/* Console Logs — согласовано со Step3Simulation.vue */
.console-logs {
  background: #000;
  color: #DDD;
  padding: 16px;
